...
var NRS = (function(NRS, $) {
	NRS.automaticallyCheckRecipient = function() {
		...
		$recipientFields.on("checkRecipient", function() {
			...
			if (value && value != "GEC-____-____-____-_____") {
			...
			if (response.publicKey) {
				...
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": $.t("recipient_malformed") + (!/^(GEC\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
							"account": null
						});
	...
	NRS.checkRecipient = function(account, modal) {
		...
		if (/^(GEC\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
